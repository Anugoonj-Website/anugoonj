...
function App() {
  ...
  const [loader_vidio_display, setLoaderVidioDisplay] = useState("none");
  const [loader_gif_display, setLoaderGifDisplay] = useState("flex");
  ...
  const loader_video = () => {
    setLoaderVidioDisplay("block");
    setLoaderGifDisplay("none");
  }
...

      {
        loading ?
          <div className="video_div">
            <video ref={website_loader} className="anugoonj_loader" style={{display:loader_vidio_display}}  playsInline muted autoPlay onLoadedData={loader_video} onEnded={() => { setLoading(false) }}>
              <source src={loader} type="video/mp4" />
            </video>

            <div className="loader_gif" style={{display:loader_gif_display}}>
              <img src={Loader_gif} alt="Loding...." />
              <h3>Loding....</h3>
            </div>
